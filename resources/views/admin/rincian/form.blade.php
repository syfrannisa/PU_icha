...
<form action="{{ ($status == 'Tambah') ? route('rincian.store') : route('rincian.update',$data->id) }}" method="POST" enctype="multipart/form-data">
@csrf
@if($status == 'Edit')
@method('patch')
@endif
...
<div class="form-group">
	<label>Sub Kegiatan</label>
		<select class="form-control" name="id_sub_kegiatan">
  		...
  		@foreach($subkegiatan as $result)
  		<option value="{{ $result->id }}" @if($status == 'Edit') @if($data->id_sub_kegiatan == $result->id) selected  @endif @endif>{{ $result->nama_sub_kegiatan }}</option>
  		@endforeach
	</select>
</div>
<div class="form-group">
   	<label>Nama Rincian</label>
   	<input type="text" class="form-control" name="nama_rincian" @if($status == 'Edit') value="{{ $data->nama_rincian}}" @endif placeholder="Inputkan Nama Rincian">
</div>
<div class="form-group">
